---
name: LLM Integration Roadmap
overview: Создание roadmap для интеграции backend с LLM через OpenRouter API, включая конфигурацию, формализацию языков программирования, тем, уровней сложности, типов ответов и промптов для генерации вопросов.
todos:
  - id: create-roadmap-file
    content: "Создать файл todo/llm.md с полным roadmap: список языков, темы для каждого языка, формализация уровней сложности, типы ответов, промпт для генерации"
    status: pending
  - id: create-env-example
    content: Создать backend/.env.example с переменными окружения для OpenRouter (API_KEY, MODEL, BASE_URL, TIMEOUT, MAX_TOKENS)
    status: pending
---
...
Использовать `github.com/revrost/go-openrouter` (стабильная версия v1.1.5) для работы с OpenRouter API.
...
2. **`backend/.env.example`** - пример конфигурации